...
Short recorded safety briefings — a free, core feature (not gated behind the Audit add-on). Admin or any technician can log one (topic, date, presenter, notes); a trainee can only view and sign. Every attendee signs their **own** attendance, on their own device — same device/IP fraud-tracking shape as logbook sign-offs.

- **New collection `toolboxTalks/{id}`** — the talk record itself (topic/date/presenter/notes) is a **direct client write**, governed by `firestore.rules` (admin or `isTechnician()`, both scoped to the caller's own company) — no IP stamping needed for this part, so no Cloud Function involved.
...
- **Re-publish `firestore.rules` and redeploy `functions`** for this to work.
