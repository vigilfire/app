...
## Company file (workshop / auditor records)

The **Company** tab keeps its letterhead profile + records-backup cards at the top, and adds a **"Workshop file"** section below them with five chip sub-tabs — the servicing business's own compliance evidence for a SAQCC / SABS audit, as distinct from the client-facing site registers and certificates on the Reports tab:
...
- **Calibration** — the **annual** calibration of the workshop's reference instruments (master gauge, test weights) by a **SANAS-accredited laboratory**. The lab (name, SANAS accreditation no., contact) is stored once on `settings/{companyId}` (`calibrationLabName` / `calibrationLabAccreditationNo` / `calibrationLabContact`); each certificate record (`calibrationCertificates` collection) holds the instrument, calibration date, certificate number, next-due date, and an upload of the certificate page. This is distinct from the **weekly verification** log on the Verify tab (see below). New `calibrationCertificates` collection.

An **expiry banner** on the tab (and the badge on the Company Documents chip) rolls up everything expired or expiring within 60 days across all five sections. A letterheaded **"Company compliance pack"** under Reports prints the whole workshop file as one document with an expiry summary.

### "Calibration" renamed to "Weekly verification"

The bottom-nav **Calib.** tab is now **Verify**, and the "Calibration register" it holds is now the **"Weekly Verification Register (SANS 1475 Annex A)"** — it always recorded the weekly checks of scales/gauges against reference instruments, which is *verification*, not calibration. This is a **UI-only rename**: the `calibrations` Firestore collection and the `canCalibrate` technician permission field are unchanged, so there is no data migration. The real (annual, external, SANAS) calibration register is the new **Calibration** chip above. A separate "Calibration register (SANAS)" report is also on the Reports tab.

- `firestore.rules`: four new match blocks — `branches`, `vehicles`, `companyDocuments`, `calibrationCertificates` — all plain company-admin (`isCompanyAdmin` on every verb; `create` checks the incoming `companyId`, the rest check the stored one). Client `list` queries filter `where('companyId','==', <own>)` so the read rule stays provable. **Re-publish `firestore.rules`.**
- `storage.rules`: one new path `company-files/{companyId}/{path=**}` covering `documents/`, `vehicles/<id>/`, `employees/<uid>/` and `branches/<id>/`. Unlike `technician-photos/` it is **admin-writable** (an admin uploads on behalf of the business), accepts `application/pdf` as well as images, and caps uploads at 15 MB. **Re-publish `storage.rules`** (`firebase deploy --only storage`).
- `firestore.indexes.json`: no change — every query is `where('companyId','==')` alone or plus one more equality, which Firestore serves without a composite index.
- Calibration certificate files go to `company-files/{companyId}/calibration/` — covered by the same storage path, no `storage.rules` change.
- `functions/index.js`: `branches`, `vehicles`, `companyDocuments`, `calibrationCertificates` added to `ACTIVITY_COLLECTIONS`, so writes bump the company's `lastActivityAt` for the /admin dashboard (`firebase deploy --only functions` — this creates `bumpActivity_branches` / `_vehicles` / `_companyDocuments` / `_calibrationCertificates`).
