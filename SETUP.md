...
The **Company** tab keeps its letterhead profile + records-backup cards at the top, and adds a **"Workshop file"** section below them with four chip sub-tabs — the servicing business's own compliance evidence for a SAQCC / SABS audit, as distinct from the client-facing site registers and certificates on the Reports tab:
...
An **expiry banner** on the tab (and the badge on the Company Documents chip) rolls up everything expired or expiring within 60 days across all four sections. A letterheaded **"Company compliance pack"** under Reports prints the whole workshop file as one document with an expiry summary.

- `firestore.rules`: three new match blocks — `branches`, `vehicles`, `companyDocuments` — all plain company-admin (`isCompanyAdmin` on every verb; `create` checks the incoming `companyId`, the rest check the stored one). Client `list` queries filter `where('companyId','==', <own>)` so the read rule stays provable. **Re-publish `firestore.rules`.**
...
- `functions/index.js`: `branches`, `vehicles`, `companyDocuments` added to `ACTIVITY_COLLECTIONS`, so writes bump the company's `lastActivityAt` for the /admin dashboard (`firebase deploy --only functions` — this creates `bumpActivity_branches` / `_vehicles` / `_companyDocuments`).
